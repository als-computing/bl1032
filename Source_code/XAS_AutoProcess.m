...
% Last Modified by GUIDE v2.5 25-Jul-2024 18:52:22
...
if get(handles.rb_newfileformat, 'value') == 1
    ...
    for k=1:data.Nre_of_SCA
        if abs(size(eval(['data.SCA_',num2str(k)]),1)-size(data.I0,1))<=2
            ...
            I0 = I0(1:indcut,:);
            %if ~strcmp(data.name{k}, 'Total') %deadtime correction not applied at the Total channel
             %M = spectemp./(I0*deadtimes)*u;
             M = spectemp./(1/deadtimes)*u;
            % else
            %  %M = spectemp./(I0*deadtimes)*1;
            %  M = spectemp./(1/deadtimes)*1;
            % end
...
% Save the difference plot figure as bmp and excel
    ...
    writetable(T,'Difference_spectrum.xls')
    saveas(hfigdiff, 'Difference spectrum', 'bmp')
...
[spectrum2, pathname2] = uigetfile('*.xls', 'Select the second processed spectrum','multiselect','off');
    ...
    writetable(T,'Divide_spectrum.xls')
    saveas(hfigdiv, 'Divide spectrum', 'bmp')
...
[spectrum2, pathname2] = uigetfile('*.xls', 'Select the second processed XFS','multiselect','off');
    ...
    set(hfigdiff,'color','white')
    movegui(hfigdiff,'west')
    leg={spectrum1, spectrum2, 'Difference'}; % this is the legend
    plot(spec1(:,1), spec1(:,2),'k')
    hold on
    plot(spec2(:,1), spec2(:,2),'r')
    xlabel('Energy keV')
...
% --- Executes on button press in pb_XSAlocation_dat.
function pb_XSAlocation_dat_Callback(hObject, eventdata, handles)
% hObject    handle to pb_XSAlocation_dat (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
if ~exist('spec2')
    ...
    writetable(S,'Average_spectrum.xls')
    saveas(hfigAv, 'Average spectrum', 'bmp')
