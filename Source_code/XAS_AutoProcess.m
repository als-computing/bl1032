...
xval = energy(1:ind_preedge-1);
 ...
 if ch == 2 % elastic
    ...
    ind_preedge = find(data.energy > energy_edgetop - cutoff,1);
    %ind_preedge = find(energy > energy_edgetop - cutoff,1);
    preedge_max = energy(ind_preedge);
    xval = energy(1:ind_preedge-1);
    enval = energy(1:ind_preedge-1);
...
[spectrum1, pathname1] = uigetfile('*.xls', 'Select a processed spectrum','multiselect','off');
    ...
    yyaxis right
    %plot(spec1(1:size(Diffspec),1), Diffspec,'r'); hold on % Non-smoothed
    plot(spec1(1:size(Diffspecsmoothed,2),1), Diffspecsmoothed','r') % smoothed
...
% Save the difference plot figure as bmp and excel
    ...
    T = array2table([spec1(1:size(Diffspecsmoothed,2),1), Diffspecsmoothed']); % smoothed
    T.Properties.VariableNames(1:2) ={'Energy (keV)','1st derivative Spectrum'};
    fname = fname(1:min(find(fname=='_',2,'last')));
    writetable(T,[fname,'1st_derivative_spectrum.xls'])
    saveas(hfigdiff, [fname,'1st_derivative_spectrum'], 'bmp')
