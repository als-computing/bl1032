...
% Last Modified by GUIDE v2.5 29-Apr-2024 12:24:58
...
set(handles.pb_saveraw_spectrum, 'visible', 'on')
...
% % Password protection - See line 6604 (or around there)
% vv = password;
% if isequal(vv, 0)
...
% --- Executes on button press in pb_saveraw_spectrum.
function pb_saveraw_spectrum_Callback(hObject, eventdata, handles)
% hObject    handle to pb_saveraw_spectrum (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
current = cd;
if get(hObject,'Value') == 1
...
if get(handles.rb_singlefile, 'value') == 1
    ...
    % Detects automatically if the file is a .qx or .dat extension file
...
file = handles.XASfile;
    ...
    if ind2plot(1) == 1 && data.DenominatorPlot(1,15) == 1
        ...
        for j=1:size(ind2plot,2)
            k = ind2plot(j);
            a = Intensity{1,k}./Intensity{1,1};
            % Initial = average of the 3 first lines
            % End = average of the 3 last lines
            if size(a,1)>=3
...
k=1;
while 1
    [names,pathname] = uigetfile('*.xls','Choose all xls file to stack', 'MultiSelect', 'on');
...
% Save the stack plot figure as bmp
[name, pathname] = uiputfile('*.bmp','Save as');
if isequal(name,0) || isequal(pathname,0)
       return
else
    name = fullfile(pathname, name);
    h=msgbox(['Stacked Plot saved in ',fullfile(pathname, name)]);
    pause(5)
    saveas(hfigstack, [name,'.bmp'])
    delete(h)
end
...
[files, pathname] = uigetfile('*.qx', 'Select a MULTIPLE files','multiselect','on');
...
% else
%     name = fullfile(pathname, name);
%     h=msgbox(['Stacked RAW Plot saved in ',fullfile(pathname, name)]);
%     pause(5)
%     saveas(hfigstackraw, [name,'.bmp'])
%     delete(h)
% end
...
if size(Intensity{1},1)>1 % condition added to deal with old .dat files
    ...
    Iomins = Ioglitch -15;
    Iomaxs = min(Ioglitch +15,max(data.energy));
    ...
    Eshift = energy(I(1))*1000 - Ioglitch; 
...
else  % Multispectra selected - plot the average and do analysis on the average
...
% ind_edgebottom = find(energy>energy(ind_edgetop)-cutoff/1000,1);
% 
% % Determine the pre-edge energy range: linear fit to isolate the intersection with the baseline
% [yfit, S, mu] = polyfit(energy(ind_edgebottom:ind_edgetop), spec(ind_edgebottom:ind_edgetop), 1);
% yval = polyval(yfit, energy(ind_edgebottom:ind_edgetop), [], mu);
% preedge_max = (min(spec)-yfit(2))/yfit(1); % this number is negative; needs to be shifted by the first value of energy(ind_edgebottom)
% preedge_max = preedge_max + 1000*energy(ind_edgebottom);
% preedge_max = preedge_max/1000;  
% 
% % fit the pre-edge data with a polynome of degree 12
% ind_preedge = find(energy > preedge_max,1);

% value for preedge cutoff - new as of 04.24
cutoff = str2double(get(handles.ed_predgecutoff, 'string'));
...
xval = energy(1:ind_preedge-1);
 ...
 if ch == 2 % elastic
    ...
    cutoff = str2double(get(handles.ed_predgecutoff, 'string'));
    rangetest = -15:20;
    %ensure, we dont encroach in E0 which will make the fit infinite
    ...
    %for display only - extend fit to larger energy
...
%----
% open the pre-edge fit in a different figure window
...
% open the postedge fit in a different figure window
...
%b = (postspec(indps:size(postspec,2))- yval)+postspec(indps);
...
% call the XAS standards and shape them
...
if isempty(SM.data)
  ...
  current = cd;
    if savech == 1
        ...
        %check if the directory to save the figure and excel file exist.
        % if not create it
        current = cd;
        cd(get(handles.edit_currentdirectory,'string'))
       if get(handles.rb_multipleload,'value')==0 % save single file corrected
           ...
           delete(hptemp)
            ...
            cd(current)
       else  % save multispectra average - process spectrum corrected            
            name_ext = [];
            for p = 1:size(handles.XASfileAv,2)
                nn=handles.XASfileAv{1,p};
                name_ext = [name_ext, nn(1,ind-4:ind-1)];
        ...
        hm2 = msgbox('No data or figures have been saved');
        pause(5)
        delete(hm2)
    end
   cd(current)
...
% here we run the name_ext only if there are multiple spectra load selected
if get(handles.rb_multipleload, 'value')==1
    name_ext = [];
    for p = 1:size(handles.XASfileAv,2)
        nn=handles.XASfileAv{1,p};
...
ind = find(handles.XASfile == '.');
 ...
 if get(handles.rb_multipleload,'value')==0 % save single file corrected % Single file processed and fitted
    ...
    % Save best fit & residuals to the experimental spectrum
        BFR = array2table([Energyscale, BfOptimized',Residual(:,combo)]);
        BFR.Properties.VariableNames(1:3) ={'Energy (keV)','Best Fit Normalized Absorption','Residuals'};
    ...
    % make a ghost figure to save it in bmp
        ...
        delete(htemp)
 else % save the data for multiple file averaged, processed and saved
     name_ext = [];
            for p = 1:size(handles.XASfileAv,2)
                nn=handles.XASfileAv{1,p};
                name_ext = [name_ext, nn(1,ind-4:ind-1)];
 ...
 %Put the default cutoff edge bCK
...
% figure('color', 'white')
    ...
    for k = 1:size(E,1)
      ...
      % Replot the spectra with the pre-edge substracted
...
%Erase the existing "Major components" Table if it exists
%Then create a new table for the current analysis
...
% --- Executes on button press in pb_XRFmaps.
function pb_XRFmaps_Callback(hObject, eventdata, handles)
% hObject    handle to pb_XRFmaps (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
[files, pathname] = uigetfile('*.xrf', 'Select a SINGLE file or TWO files','MultiSelect', 'on');
cd(current)
 ...
 for q=1:scans
     ...
     if get(handles.rb_newfileformat, 'value') == 1
        ...
        data = openmapfile_original([pathname,file]);
     end
    ...
    [indx2use,tf] = listdlg('ListString',data.name ,'PromptString','Select the elements to map:',...
...
indx2use = handles.indx2use; % this is the indx of the elements to use
i=1; % running index
...
% Pick three elements to produce a tricolor map
indactive = find(active == 1);
if isempty(indactive)
    msgbox('Run XRF Map auto first')
    return
...
% booklet. T is a cell of N row and 2 columns for N elements, each row as
...
if get(handles.rb_spc_single, 'value') == 1 || get(handles.rb_mca_single, 'value') == 1
    data = [];
% upload spc files - assume spectra are calibrated
  ...
  % for the .mca files - will calibrate energy
...
%open the first file to find out what element for the chemical maps
cd(current)
file = files(1).name;
if get(handles.rb_newfileformat, 'value') == 1
...
% Give option to select the element to use among a list of all elements
  ...
  % square
   ...
   % The error bars are normalized to a fixed value of Z microns
    ...
    % Set zoom on reference map
      ...
      %Rectangle to mask additional points to make the matrix
      %square
                rectangle('position',[NbrePixel_X+1, 0, NbrePixel_Y-NbrePixel_X+1,NbrePixel_Y+1], 'facecolor', 'w', 'edgecolor', 'w') 
       elseif NbrePixel_X > NbrePixel_Y
                    hFig = figure('name', file,'units','normalized','NumberTitle', 'off', 'color', 'white');
...
%Set the zoom on the reference map
...
% Plot files at different energies.
...
%         % overplot a white rectangle to make the figure square
%         if size(C,1) < size(C,2) 
%             imagesc([C_offset;A]);colormap 'turbo'; colorbar('northoutside')
%             hold on
%             rectangle('position',[0, size([C_offset;A],1)-Ad, size([C_offset;A],1)+1,Ad+1], 'facecolor', 'w', 'edgecolor', 'w')
%             % stretched in X, add a rectangular to the bottom
%         else
%             imagesc([C_offset,A]);colormap 'turbo'; colorbar('northoutside')
%             hold on
%             rectangle('position',[size([C_offset,A],1)-Ad,0, Ad+1, size(C_offset,1)+1], 'facecolor', 'w', 'edgecolor', 'w')
%             % stretched in Y, add a rectangle to the right
%         end
%         text(txtpos(1), txtpos(2), data.name{k},'FontName','Arial','Fontsize',fs, 'fontweight', 'bold', 'BackgroundColor', 'white')
%         zoom on
%         axis off
%         axis square 
        ...
        if ~isequal(numb, 100)
         ...
         if size(C,1) < size(C,2) 
            ...
            j = strfind(file, '.');
              ...
              % replace the '.' by '_' otherwise the file extension is
              % screwed
                if size(j,2)>1   
                    pp=j(1:size(j,2)-1);
                    for q=1:size(pp,2)
...
% check if the cross-correlation will be performed across tow different
% energy scan. This is the case if th
  
% Button menu to select if cross-correlation is done across mutli-energy
...
% Maps with XAS point locations added
% --- Executes on button press in pb_XASlocation.
function pb_XASlocation_Callback(hObject, eventdata, handles)
% hObject    handle to pb_XASlocation (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% XAS point locations on the XRF maps
% open the .qx files
% First identify a directory where to look for the data files
default_directory = get(handles.edit_defaultdirectory, 'string');
current_directory = get(handles.edit_currentdirectory, 'string');
...
% maps with XFS point locations added
...
% XFS point locations on the XRF maps
% open .mca files only
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Below is the part that create the passcode protection
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
MyPassword = 'xray1032';        % SET IT HERE MANUALLY - the code must be inbetween '......'
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
